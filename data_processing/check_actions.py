...
if __name__ == "__main__":
    ...
    for topic, msg, t in bag.read_messages():
        if topic == '/gscam/image_raw':
            goodMove = imgToMove(msg)
        elif topic == '/constant_move_times':
            sentMove = armCmdToMove(msg)
            #Check if we did the right thing or not
            if sentMove == "L":
                lMoves += 1
                if goodMove == "L":
                    lCorrect += 1
                ...
                if goodMove == "R":
                    rCorrect += 1
    print outfileroot +",{0},{1},{2},{3}".format(lMoves,lCorrect,rMoves,rCorrect)