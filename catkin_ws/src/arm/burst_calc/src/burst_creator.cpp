...
void BurstCreator::init()
{
    ...
    ROS_INFO("Burst creator got a subscriber.");
...
#ifdef MERGE
            	//We're not done here, add it to the merger.
                merger_.add(bursts_[i].getBurst());
#else
                //We're done here, publish it
                burst_pub_cat_.publish(bursts_[i].getBurst());
                ROS_INFO("Burst %.3f published", bursts_[i].getBurst().header.stamp.toSec());
#endif
                bursts_[i].reset();
            }
        }
...
#ifdef MERGE
        //This is all totally unneeded if you published the message up there-^.
        merger_.update();
...
#else
    	// Grab any remaining bursts that are hanging around and publish them
        if (bursts_[i].isBursting())
        {
            printf("*   Burst   : [Sz %4d] [%6.3f - %6.3f] [Ch %d]\n",
                   static_cast<int>(bursts_[i].getBurst().dishes.size()),
